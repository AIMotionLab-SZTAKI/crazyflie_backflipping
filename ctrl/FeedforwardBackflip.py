...
class FeedforwardBackflip:
    """Backflip with feedforward control."""
    def __init__(self, model, data):
        ...
        self.Umax = 0.5 / self.mass
        self.Umin = 0.1 / self.mass
        self.max_ang_vel = 23  # rad/s
        self.gravity = 9.81
        self.model = model
        self.data = data
        self.control_step = 0.005

    def compute_control_sequence(self, params):
        U1, t1, t3, U5, t5 = params
        alpha = 2*self.Ixx/self.mass/self.length/self.length
        p1 = -(self.Umax - U1)/alpha/self.length
        p2 = (self.Umax - self.Umin) / alpha / self.length / 2
        p3 = 0
        p4 = -1 * p2
        U2 = (self.Umax + self.Umin) / 2
        t2 = (self.max_ang_vel - p1 * t1) / p2
        U4 = U2
        U3 = self.Umin
        p5 = (self.Umax - U5) / alpha / self.length
        t4 = -(self.max_ang_vel + p5 * t5) / p4
        num_steps = [int(t/self.control_step) for t in (t1, t2, t3, t4, t5)]
        f = np.hstack([U * np.ones(num_steps_) * self.mass for U, num_steps_ in
                       zip((U1, U2, U3, U4, U5), num_steps)])
        tau = np.hstack([p * np.ones(num_steps_) * self.Ixx for p, num_steps_ in
                         zip((p1, p2, p3, p4, p5), num_steps)])
        return f, tau
